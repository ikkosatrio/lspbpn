...
class Gallery extends Model
{
    ...
    {
        $url = url('/')."/public/image/gallery/".$this->image;
        return $url;
    }

    public function ImagePathLocal()
    {
        $url = public_path("image/gallery/".$this->image);
        return $url;
    }
    ...
    public function getImagePathSmallAttribute(){
        ...
        if(isset($path_parts['filename'])){
            ...
            $urlImage = public_path("image/gallery/".$path_parts['filename']."_small.png");
            // dd($urlImage);
            ...
            if(!$fileexist){
                ...
                $img->save('public/image/gallery/'.$path_parts['filename']."_small.png",50);
                // $img->encode('png');
                // $type = 'png';
                // $base64 = 'data:image/' . $type . ';base64,' . base64_encode($img);
            }
            return url('/')."/public/image/gallery/".$path_parts['filename']."_small.png";
        }else{
            return url('/')."/public/image/gallery/".$this->image;
        }
    }
    ...
    public function getImagePathMediumAttribute(){
        ...
        if(isset($path_parts['filename'])){
            ...
            $urlImage = public_path("image/gallery/".$path_parts['filename']."_medium.png");
            // dd($urlImage);
            $fileexist = File::exists($urlImage);
            if(!$fileexist){
                ...
                $img->save('public/image/gallery/'.$path_parts['filename']."_medium.png",50);
                // $img->encode('png');
                // $type = 'png';
                // $base64 = 'data:image/' . $type . ';base64,' . base64_encode($img);
            }
            return url('/')."/public/image/gallery/".$path_parts['filename']."_medium.png";
        }else{
            return url('/')."/public/image/gallery/".$this->image;
        }
    }

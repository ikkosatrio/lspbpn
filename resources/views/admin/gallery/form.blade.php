        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form id="formInput" action="{{ ($data['typeForm'] =="create") ? route('admin.gallery.store') : route('admin.gallery.update',$data['dataModel']->id) }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-lg-10">
                                                <textarea id="editor" rows="3" cols="3" class="form-control" name="description" placeholder="Description">{{ ($data['typeForm'] =="create") ? "" : $data['dataModel']->description }}</textarea>
                                            </div>
                                        </div>
                                                ...
                                                @if(isset($data['category']))
                                                    @foreach ($data['category'] as $item)
                                                        @if($data['typeForm'] != "create")
                                                            <option {{($data['dataModel']->category_id == $item->id) ? "selected" : ""}} value="{{$item->id}}">{{$item->name}}</option>
                                                        @else
...
<script>
    $(document).ready(function(){
        ...
        var options = {
            ...
            extraPlugins : 'justify,youtube,mathjax',
        };
